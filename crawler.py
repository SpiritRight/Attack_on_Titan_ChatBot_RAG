...
# 진격 중학교 관련 페이지 제거
EXCLUDED_URLS: List[str] = ["https://namu.wiki/w/%EC%A7%84%EA%B2%A9!%20%EA%B1%B0%EC%9D%B8%20%EC%A4%91%ED%95%99%EA%B5%90/%EB%93%B1%EC%9E%A5%EC%9D%B8%EB%AC%BC", 
                            "https://namu.wiki/w/%EB%B6%84%EB%A5%98:%EC%A7%84%EA%B2%A9!%20%EA%B1%B0%EC%9D%B8%20%EC%A4%91%ED%95%99%EA%B5%90",
                            "https://namu.wiki/w/%EA%BF%87%EC%96%B4%EB%9D%BC%20%EB%8F%BC%EC%A7%80%EB%93%A4%EC%95%84"]
...
def load_exclude_urls(exclude_file: Optional[str], exclude_urls: List[str]) -> Set[str]:
    excluded: Set[str] = set()
    for url in exclude_urls:
        if url:
            excluded.add(normalize_url(url))
    if exclude_file:
        with open(exclude_file, "r", encoding="utf-8") as handle:
            for line in handle:
                ...
                if not line or line.startswith("#"):
                    continue
                excluded.add(normalize_url(line))
    return excluded
...
def extract_structured_items(driver: webdriver.Chrome) -> List[Dict[str, object]]:
    ...
    try {
        ...
        return elements.map(el => {
            ...
            if (el.closest('nav, header, footer, aside, .wiki-toc, .wiki-toc-wrapper, .no0DxkqA._1m7kazWY, .WMQhH6LY._0ahwpliD, .F83V3QG7')) {
                return null;
            }
...
def chunk_blocks(
    ...
    def emit_single(
        ...
        append_chunk(text, section, is_table=is_table, is_quote=is_quote)
        current = []
        current_len = 0
    ...
    for block in blocks:
        ...
        if block_type in {"table", "blockquote"}:
            emit_single(
                ...
                is_quote=(block_type == "blockquote"),
            )
            continue
...
def crawl(
    ...
    seen: Set[str] = set()
    visited = 0
    exclude_urls = {normalize_url(u) for u in exclude_urls if u}
    queue: deque[str] = deque(
        [normalize_url(u) for u in start_urls if u and normalize_url(u) not in exclude_urls]
    )
    ...
    try:
        while queue and visited < max_pages:
            ...
            print(f"[{visited}/{max_pages}] crawling: {url}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Exclude a specific URL. Repeat for multiple URLs.",
    )
    parser.add_argument(
        "--exclude-file",
        default=None,
        help="Path to a file with URLs to exclude, one per line.",
    )
    parser.add_argument(
        "--include-category",
        action="store_true",
        ...
        for item in results:
            ...
            for chunk in chunks:
                payload = {
                    ...
                    "is_quote": bool(chunk.get("is_quote")),
                }
...
def main() -> None:
    ...
    exclude_urls = load_exclude_urls(args.exclude_file, args.exclude_url)
    exclude_urls.update(normalize_url(u) for u in EXCLUDED_URLS if u)
    results = crawl(
